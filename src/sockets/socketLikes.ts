...
export class Socket_io_Likes {
    ...
    async createLikes(socket: Socket, { id_evento, likes, nro_documento_usuario }: LikesPos) {
        try {
            const request = pool.request()
    ...
    async deleteLikes(socket: Socket, { id_evento, nro_documento_usuario }: LikesDelete) {
        try {
            const request = pool.request()
                .input('id_evento5', sql.Int, id_evento)
